...
namespace Axe.TaskManagement.Service.Services.Implementations
{
    ...
    public partial class JobService
    {
        ...
        public async IAsyncEnumerable<JobDto> GetStreamingHistoryJobByUserForExport(PagingRequest request, string actionCode, string accessToken)
        {
            #region filter & short
            ...
            #endregion
            var findOptions = new FindOptions<Job> { BatchSize = 1000 }; // Lấy 1.000 bản ghi/lần
            using var cursor = await _repository.GetCursorListJobAsync(lastFilter, findOptions);

            while (await cursor.MoveNextAsync())
            {
                foreach (var job in cursor.Current)
                {
                    yield return _mapper.Map<JobDto>(job); // Chuyển đổi từng Job sang JobDto
                }
            }
        }
