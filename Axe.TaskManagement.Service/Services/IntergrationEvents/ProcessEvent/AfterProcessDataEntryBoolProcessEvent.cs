...
{
    public interface IAfterProcessDataEntryBoolProcessEvent : IBaseInboxProcessEvent<AfterProcessDataEntryBoolEvent>, IDisposable { }

    public class AfterProcessDataEntryBoolProcessEvent : Disposable, IAfterProcessDataEntryBoolProcessEvent
    {
        ...
        public AfterProcessDataEntryBoolProcessEvent(
        ...
        public async Task<Tuple<bool, string, string>> ProcessEvent(AfterProcessDataEntryBoolEvent evt, CancellationToken ct = default)
        {
            while (true)
            {
                ...
                try
                {
                    ...
                    foreach (var job in jobs)
                    {
                        ...
                        // 1.1. Tạo Transaction thanh toán cho worker & hệ thống: Nếu bước SAU là DataCheckBoolAuto thì chỉ là tiền TẠM TÍNH, ngược lại thì là tiền THỰC
                        if (clientInstanceId != Guid.Empty && !job.IsIgnore)
                        {
                            bool isNextStepDataConfirm = nextWfsInfoes != null && nextWfsInfoes.Count == 1 && nextWfsInfoes.FirstOrDefault(x => x.ActionCode == ActionCodeConstants.DataConfirmBoolAuto) != null;
                            bool isContainStepDataConfirm = wfsInfoes.FirstOrDefault(x => x.ActionCode == ActionCodeConstants.DataConfirmBoolAuto) != null ? true : false;
                            if (isContainStepDataConfirm && !isNextStepDataConfirm) //=> không tồn tại DataCheck hoặc DataConfirm ngay sau
                            {
                            ...
                            {
                                SourceUserInstanceId = (isContainStepDataConfirm && isNextStepDataConfirm) ? Guid.Empty : clientInstanceId,
                                DestinationUserInstanceId = userInstanceId,
                                ChangeAmount = (isContainStepDataConfirm && isNextStepDataConfirm)
                                    ? 0
                                    : job.Price * (100 - job.ClientTollRatio) / 100,
                                ChangeProvisionalAmount = (isContainStepDataConfirm && isNextStepDataConfirm)
                                    ? job.Price * (100 - job.ClientTollRatio) / 100
                                    : 0,
                                JobCode = job.Code,
                                ProjectInstanceId = job.ProjectInstanceId,
                                ...
                                Description = (isContainStepDataConfirm && isNextStepDataConfirm)
                                    ? string.Format(
                                        DescriptionTransactionTemplate
                                            .DescriptionIncreaseProvisionalMoneyForCompleteDataEntryJob,
                                        userInstanceId, job.Code)
                                    : string.Format(
                        ...
                        {
                            Log.Information($"ProcessDataEntryBool change step: DocInstanceId => {job.DocInstanceId}; ActionCode => {job.ActionCode}; WorkflowStepInstanceId => {job.WorkflowStepInstanceId}");
                        ...
                        var itemInputParams = new List<ItemInputParam>
                {
                            ...
                            if (nextWfsInfoes.All(x => x.ActionCode != ActionCodeConstants.End))
                            {
                                ...
                                foreach (var nextWfsInfo in nextWfsInfoes)
                                {
                                    ...
                                    if (isTriggerNextStep)
                                    {
                                        ...
                                        var resultDocChangeCurrentWfsInfoChild = await _docClientService.ChangeCurrentWorkFlowStepInfo(job.DocInstanceId.GetValueOrDefault(), crrWfsInfo.Id, (short)EnumJob.Status.Complete, job.WorkflowStepInstanceId.GetValueOrDefault(), null, string.Empty, null, accessToken: accessToken);
                                        if (!resultDocChangeCurrentWfsInfoChild.Success)
                                        {
                                            Log.Logger.Error($"{nameof(AfterProcessDataEntryBoolProcessEvent)}: Error change current work flow step info for DocInstanceId: {job.DocInstanceId.GetValueOrDefault()} !");
                                        }
                    ...
                    // 1.2. Cập nhật thanh toán tiền cho worker bước HIỆN TẠI (DataEntryBool)
                    ...
                    if (jobEnds.Any())
                    {
                        ...
                        foreach (var docInstanceId in docInstanceIds)
                        {
                            ...
                            var wfsId = jobEnds.FirstOrDefault(x => x.DocInstanceId == docInstanceId)?.Id;
                            ...
                            if (!hasJobWaitingOrProcessing)
                            {
                                ...
                                if (!resultDocChangeCurrentWfsInfoChild.Success)
                                {
                                    Log.Logger.Error($"{nameof(AfterProcessDataEntryBoolProcessEvent)}: Error change current work flow step info for DocInstanceId: {docInstanceId} !");
                                }
                            }
                        }
                    }
