...
namespace Axe.TaskManagement.Service.Services.IntergrationEvents.EventHanding
{
    ...
    {
        private readonly IAfterProcessDataCheckProcessEvent _afterProcessDataCheckProcessEvent;
        ...
        public AfterProcessDataCheckIntegrationEventHandler(
            ...
            IAfterProcessDataCheckProcessEvent afterProcessDataCheckProcessEvent,
            ...
            IJobRepository repository)
        {
            ...
            _afterProcessDataCheckProcessEvent = afterProcessDataCheckProcessEvent;
            ...
            if (@event != null && ((@event.Jobs != null && @event.Jobs.Any()) || (@event.JobIds != null && @event.JobIds.Any())))
            {
                ...
                if (isInsertSuccess)
                {
                    if (isProcessImmediate)
                    {
                        ...
                        try
                        {
                            var result = await _afterProcessDataCheckProcessEvent.ProcessEvent(@event, ct);
                            ...
                            else
                            {
                                ...
                                sw.Stop();
                                Log.Logger.Information($"Not Acked {nameof(AfterProcessDataCheckEvent)} with JobIds: {jobIds} - Elapsed time {sw.ElapsedMilliseconds} ms");
                            }
                        }
