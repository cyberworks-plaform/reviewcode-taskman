...
namespace Axe.TaskManagement.Service.Services.IntergrationEvents.EventHanding
{
    ...
    {
        private readonly IAfterProcessDataConfirmProcessEvent _afterProcessDataConfirmProcessEvent;
        ...
        public AfterProcessDataConfirmIntegrationEventHandler(
            ...
            IAfterProcessDataConfirmProcessEvent afterProcessDataConfirmProcessEvent,
            ...
            IJobRepository repository)
        {
            ...
            _afterProcessDataConfirmProcessEvent = afterProcessDataConfirmProcessEvent;
            ...
            if (@event != null && ((@event.Jobs != null && @event.Jobs.Any()) || (@event.JobIds != null && @event.JobIds.Any())))
            {
                ...
                if (isInsertSuccess)
                {
                    if (isProcessImmediate)
                    {
                        ...
                        try
                        {
                            var result = await _afterProcessDataConfirmProcessEvent.ProcessEvent(@event, ct);
                            ...
                            if (isAck)
                            {
                                ...
                                sw.Stop();
                                Log.Logger.Information($"Acked {nameof(AfterProcessDataConfirmEvent)} with JobIds: {jobIds} - Elasped time {sw.ElapsedMilliseconds} ms");
                            }
                            else
                            {
                                ...
                                sw.Stop();
                                Log.Logger.Information($"Not Acked {nameof(AfterProcessDataConfirmEvent)} with JobIds: {jobIds} - Elasped time {sw.ElapsedMilliseconds} ms");
                            }
                        }
