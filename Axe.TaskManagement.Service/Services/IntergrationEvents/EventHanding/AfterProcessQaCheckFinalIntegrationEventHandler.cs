...
namespace Axe.TaskManagement.Service.Services.IntergrationEvents.EventHanding
{
    ...
    {
        private readonly IAfterProcessQaCheckFinalProcessEvent _afterProcessQaCheckFinalProcessEvent;
        ...
        public AfterProcessQaCheckFinalIntegrationEventHandler(
            ...
            IAfterProcessQaCheckFinalProcessEvent afterProcessQaCheckFinalProcessEvent,
            ...
            IJobRepository repository)
        {
            ...
            _afterProcessQaCheckFinalProcessEvent = afterProcessQaCheckFinalProcessEvent;
            ...
            if (@event != null && (@event.Job != null || !string.IsNullOrEmpty(@event.JobId)))
            {
                ...
                if (isInsertSuccess)
                {
                    if (isProcessImmediate)
                    {
                        ...
                        try
                        {
                            var result = await _afterProcessQaCheckFinalProcessEvent.ProcessEvent(@event, ct);
                            ...
                            else
                            {
                                ...
                                sw.Stop();
                                Log.Logger.Information($"Not Acked {nameof(AfterProcessQaCheckFinalEvent)} with JobId {jobId} - Elapsed time {sw.ElapsedMilliseconds} ms");
                            }
                        }
