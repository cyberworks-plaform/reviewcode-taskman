...
{
    public class AfterProcessDataEntryIntegrationEventHandler : IIntegrationEventHandler<AfterProcessDataEntryEvent>
    {
        private readonly IAfterProcessDataEntryProcessEvent _afterProcessDataEntryProcessEvent;
        ...
        public AfterProcessDataEntryIntegrationEventHandler(
            IConsumerConfigClientService consumerConfigClientService,
            IConfiguration configuration,
            IAfterProcessDataEntryProcessEvent afterProcessDataEntryProcessEvent,
            ...
            IJobRepository repository)
        {
            ...
            _afterProcessDataEntryProcessEvent = afterProcessDataEntryProcessEvent;
        ...
        public async Task Handle(AfterProcessDataEntryEvent @event)
        {
            if (@event != null && ((@event.Jobs != null && @event.Jobs.Any()) || (@event.JobIds != null && @event.JobIds.Any())))
            {
                ...
                Log.Logger.Information($"Start handle integration event from {nameof(AfterProcessDataEntryEvent)} with JobIds: {jobIds}");
                ...
                if (isInsertSuccess)
                {
                    if (isProcessImmediate)
                    {
                        ...
                        try
                        {
                            var result = await _afterProcessDataEntryProcessEvent.ProcessEvent(@event, ct);
                            ...
                            if (isAck)
                            {
                                ...
                                sw.Stop();
                                Log.Logger.Information($"Acked {nameof(AfterProcessDataEntryEvent)} with JobIds: {jobIds} - Elapsed time {sw.ElapsedMilliseconds} ms");
                            }
                            else
                            {
                                ...
                                sw.Stop();
                                Log.Logger.Information($"Not Acked {nameof(AfterProcessDataEntryEvent)} with JobIds: {jobIds} - Elapsed time {sw.ElapsedMilliseconds} ms");
                            }
                        }
            ...
            {
                Log.Logger.Error($"{nameof(AfterProcessDataEntryEvent)}: @event is null!");
            }
        }
